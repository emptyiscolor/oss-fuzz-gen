...
OSSFUZZGEN_BENCHMARK_DIR="/mydata/data/code/fuzzing/oss-fuzz-gen/benchmark-seedgen/"
SCRIPT_TIMEOUT=30s
...
for proj_path in $(ls $OSSFUZZGEN_BENCHMARK_DIR); do
  ...
  mkdir -p $proj_name/aigen_corpus
  for pyfile in $(ls $OSSFUZZGEN_BENCHMARK_DIR/$proj_path/*.py); do
    ...
    mkdir -p $proj_name/aigen_corpus/$script_out_dir
    pushd $proj_name/aigen_corpus/$script_out_dir
